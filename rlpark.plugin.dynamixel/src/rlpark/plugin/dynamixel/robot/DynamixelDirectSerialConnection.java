...
public class DynamixelDirectSerialConnection implements IRobotObservationReceiver, SerialPortEventListener {
  ...
  DynamixelDirectSerialConnection(String serialPortPath, byte[] motorIDs) {
  ...
  synchronized private void receiveMessage(byte[] bytes) {
    int nbAvailable = 0;
  ...
  @Override
  synchronized public void sendMessage(byte[] bytes) {
    try {
      output.write(bytes);
      output.flush();
    } catch (IOException e) {
